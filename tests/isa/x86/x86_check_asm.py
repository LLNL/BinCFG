...
from bincfg.normalization.norm_utils import imm_to_int
from bincfg.normalization.x86.x86_tokenizer import Tokens
import re
import os


def x86_check_assembly_rules(token_list, newline_tup):
...
def _check_instruction(token_list):
    ...
    # Allow for single 'lock' instructions
    if instr_as_str == '_' and any(t[1] == 'lock' for t in token_list):
        return
    if m is None:
    ...
    for op_match in re.finditer(INSTRUCTION_OPERAND, groups['operands']):
        ...
        if op_groups['register'] is not None:
            ...
            start_idx = _scan_for_token((Tokens.IMMEDIATE, Tokens.STRING_LITERAL), token_list, start_idx=start_idx, ret_idx=True)[1] + 1
        ...
        # Memory expressions will be handled specially
            ...
            start_idx = _scan_for_token([Tokens.MEMORY_SIZE, Tokens.SEGMENT, Tokens.OPEN_BRACKET], token_list, 
...
X86_64_ISA = _read_instruction_set(os.path.join(os.path.dirname(__file__), 'x86_64_isa.txt'))
