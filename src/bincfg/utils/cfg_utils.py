...
def check_for_normalizer(dataset, cfg_data):
    ...
    Args:
        ...
        cfg_data (Iterable[Union[str, CFG, MemCFG, CFGDataset, MemCFGDataset]]): an iterable of 
            str/``CFG``/``MemCFG``/``CFGDataset``/``MemCFGDataset``'s

    Raises:
        ValueError: when there are multiple conflicting normalizers, or if no normalizer could be found
    """
    # If this is an individual object whose normalizer is the same for itself or all sub-objects, then we only need to check itself
    if isinstance(cfg_data, (bincfg.MemCFG, bincfg.CFG, bincfg.MemCFGDataset)) or \
        (isinstance(cfg_data, bincfg.CFGDataset) and not cfg_data.allow_multiple_norms):
        cfg_data = [cfg_data]
