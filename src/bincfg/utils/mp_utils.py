...
# Possible thread pool for multiprocessing, and queue to send data between threads
THREAD_POOL = None
THREAD_QUEUE = None
...
def init_thread_pool(num_workers=None, init_queue=True):
    """Initializes the thread pool.

    Args:
        num_workers (int, optional): number of processes. Will default to min(os.cpu_count(), MAX_DEFAULT_NUM_WORKERS) 
            if None. Defaults to None.
        init_queue (bool): if True and the thread pool is initialized, then a queue will be initialized alongside it
            to allow for message passing between processes. This needs to be initialized before the thread pool is
            initialized because multiprocessing says so
    """
    global THREAD_POOL, _THREAD_POOL_INITIALIZED, THREAD_POOL_NUM_WORKERS, THREAD_QUEUE
...
def get_thread_pool(ret_n=False, ret_queue=False, num_workers=None, init_queue=True):
    """Returns (starts if needed) the current thread pool.

    Args:
        ret_n (bool): if True, also returns the number of workers. Defaults to False.
        ret_queue (bool): if True, will also return the thread queue. Defaults to False.
        num_workers (Optional[Union[int, None]]): if not None, then the number of workers to use when initializing the 
            thread pool. Only used if the thread pool is not currently initialized. Defaults to None.
        init_queue (bool): if True and the thread bool gets initialized in this call, then this determines whether or
            not to initialize a queue alongside for message passing between processes

    Returns:
        Union[Pool, Tuple[Pool, int], Tuple[Queue, int], Tuple[Pool, Queue, int]]: the pool, (pool, queue), 
            (pool, num_workers), (pool, queue, num_workers) depending on what is returned
    """
    if not _THREAD_POOL_INITIALIZED:
        init_thread_pool(num_workers=num_workers, init_queue=init_queue)
    
    return THREAD_POOL if not ret_n and not ret_queue else \
        ([THREAD_POOL] + ([THREAD_QUEUE] if ret_queue else []) + ([THREAD_POOL_NUM_WORKERS] if ret_n else []))


def get_thread_queue():
    """Returns the thread queue used to pass messages between processes"""
    return THREAD_QUEUE
    ...
    def _atomic_update(self, token_dict=None):
        ...
        Args:
            token_dict (Optional[Dict[str, int]]): token dictionary to update with, or None to just read in any updated
                tokens from file
        """
        self._data.atomic_update(update_atomic_tokens, token_dict if token_dict is not None else {})
